...
scatterplot_score <- ggplot(data_long, aes(x = Test, y = Score, color = Test)) +
  ...
  labs(title = "Scatter plot of pre-test and post-test scores",
       x = "Test",
       y = "Score") +
  theme_minimal()

ggsave(filename = "plots/scatterplot_score.pdf", plot = scatterplot_score)

# Create a whisker box diagram with ggplot2
whisker_box_score <- ggplot(data_long, aes(x = Test, y = Score, fill = Test)) +
  geom_boxplot() +
  labs(title = "Moustache box of pre-test and post-test scores",
       x = "Test",
       y = "Score") +
  theme_minimal()

ggsave(filename = "plots/whisker_box_score.pdf", plot = whisker_box_score)
...
# Show results
print(paste("Pre-test mode :", mode_pre_test))
print(paste("Post-test mode :", mode_post_test))
