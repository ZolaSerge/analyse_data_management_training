...
# Load data
data <- read_csv("data/kisanga.csv")
data
...
scatterplot_score <- ggplot(data_long, aes(x = Test, y = Score, color = Test)) +
  ...
  labs(title = "Nuage de points du pre-test & post-test scores",
       x = "Test",
       y = "Score") +
  theme_minimal()

ggsave(filename = "plots/bcz/kisanga/scatterplot_score.pdf", plot = scatterplot_score)

# Create a whisker box diagram with ggplot2
whisker_box_score <- ggplot(data_long, aes(x = Test, y = Score, fill = Test)) +
  geom_boxplot() +
  labs(title = "Diagramme de Moustache du pre-test & post-test scores",
       x = "Test",
       y = "Score") +
  theme_minimal()

ggsave(filename = "plots/bcz/kisanga/whisker_box_score.pdf", plot = whisker_box_score)
